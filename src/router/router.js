...
export class Router {
    ...
    navigate(path, data = {}){
        if (this.has(path)) {
            const {component} = this.get(path); // pobranie funkcji strzałkowej generującej tresc strony
        component()
        .then(html=>{
        ...
        history.pushState(data, 'Tytuł strony', path );    //Aktualizujemy historie aby user mógł by się cofnąć
    }
}